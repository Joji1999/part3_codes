...
class PokerFiveCardRule implements PokerRule
{
    ...
    {
        return (count(array_unique(array_slice($cardRanks, 0, 3))) === 1 && count(array_unique(array_slice($cardRanks, 3, 2))) === 1) || (count(array_unique(array_slice($cardRanks, 0, 2))) === 1 && count(array_unique(array_slice($cardRanks, 2, 3))) === 1);
    }
